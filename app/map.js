...
// Custom 8-Bit Cypher Marker Icon
const cypherIcon = new L.Icon({
  iconUrl: '/cypher.png',
  iconSize: [40, 40],
  iconAnchor: [20, 20],
  popupAnchor: [0, -20],
...
// Map Controller Component: Handles manual free-roam vs GPS tracking lock
function MapController({ deviceCoords, isAutoFollow, setIsAutoFollow }) {
  const map = useMap();

  // Instantly drop auto-follow lock when user manually touches, drags, or zooms the map
  useEffect(() => {
    if (!map) return;
  ...
  // Fly to device coordinates ONLY when isAutoFollow toggles to TRUE
  useEffect(() => {
    if (isAutoFollow && deviceCoords && map) {
      map.flyTo(deviceCoords, 16, { animate: true, duration: 1 });
    }
  }, [isAutoFollow]); // Removed deviceCoords dependency to stop continuous snap-backs
...
export default function Map() {
  ...
  // High-accuracy Geolocation Watcher for PC and Mobile
  useEffect(() => {
    if (typeof window === 'undefined' || !navigator.geolocation) return;

    const geoOptions = {
      enableHighAccuracy: true, // Queries Wi-Fi access points on PC for street-level precision
      timeout: 10000,
      maximumAge: 0,
    };
  ...
  // Center/Re-lock camera on Cypher icon location
  const handleReCenterOnCypher = () => {
    setIsAutoFollow(true);
    if (mapRef.current && deviceCoords) {
      mapRef.current.flyTo(deviceCoords, 16, { animate: true, duration: 1 });
    }
  };
  ...
  const handleSearch = (e) => {
    ...
    // Perform search location logic here...
    setIsAutoFollow(false); // Drop auto-follow when exploring search results
  };

  // Default initial map center (Iloilo City default until GPS resolves)
  const initialCenter = deviceCoords || [10.7202, 122.5621];
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100dvh', overflow: 'hidden' }}>
      ...
      <div className="header-bar" style={{
        ...
        <img src="/cypher.png" alt="Cypher" className="header-icon" style={{ width: '24px', height: '24px' }} />
        <span className="header-title" style={{ fontWeight: 'bold', letterSpacing: '1px' }}>CYPHER TRACKER</span>
      </div>
      ...
      <div className="status-box" style={{
        ...
        transform: 'translateX(-50%)',
        zIndex: 1000,
        backgroundColor: 'rgba(10, 20, 29, 0.9)',
        border: '2px solid #1a8cff',
        borderRadius: '6px',
        padding: '6px 12px',
        width: '90%',
        ...
        <div className="status-text" style={{ fontSize: '9px', marginBottom: '4px', opacity: 0.8 }}>
          SYS.IP // {deviceCoords ? `${deviceCoords[0].toFixed(4)}, ${deviceCoords[1].toFixed(4)}` : 'ACQUIRING GPS...'} | MODE: GPS (HARDWARE)
        </div>
        <form onSubmit={handleSearch} style={{ display: 'flex', gap: '6px' }}>
          ...
          />
          <button type="submit" className="btn-ui" style={{
            backgroundColor: '#050b10', border: '1px solid #1a8cff', color: '#1a8cff', padding: '4px 8px', cursor: 'pointer', fontSize: '10px'
          }}>LOCATE</button>
          
          <button type="button" onClick={handleReCenterOnCypher} className="btn-ui" style={{
            backgroundColor: isAutoFollow ? '#1a8cff' : '#050b10',
            border: '1px solid #1a8cff',
            color: isAutoFollow ? '#0a141d' : '#1a8cff',
            padding: '4px 8px',
            cursor: 'pointer',
            fontWeight: 'bold',
            fontSize: '10px'
      ...
      >
        ...
        {deviceCoords && (
          <Marker position={deviceCoords} icon={cypherIcon}>
            <Popup>
              <div style={{ color: '#000', fontSize: '11px', fontWeight: 'bold' }}>
                CYPHER ACTIVE TARGET<br/>
                LAT: {deviceCoords[0]}<br/>
                LNG: {deviceCoords[1]}
              </div>
            </Popup>
          </Marker>
      ...
      {/* Bottom-Right RADAR HUD Button */}
      <div
        className="radar-hud"
        onClick={handleReCenterOnCypher}
        style={{
          position: 'absolute',
          bottom: '24px',
          right: '24px',
          ...
          height: '70px',
          borderRadius: '50%',
          border: `2px solid ${isAutoFollow ? '#00ffff' : '#1a8cff'}`,
          backgroundColor: 'rgba(10, 20, 29, 0.85)',
          display: 'flex',
          flexDirection: 'column',
          alignItems: 'center',
          justifyContent: 'center',
          cursor: 'pointer',
          boxShadow: isAutoFollow ? '0 0 12px #00ffff' : '0 0 6px rgba(0,140,255,0.5)',
          transition: 'all 0.2s ease',
        }}
      >
        <img src="/cypher.png" alt="Radar" className="radar-icon" style={{ width: '22px', height: '22px' }} />
        <span className="radar-text" style={{ fontSize: '8px', color: isAutoFollow ? '#00ffff' : '#1a8cff', marginTop: '2px', fontWeight: 'bold' }}>
